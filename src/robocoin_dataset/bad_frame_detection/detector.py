...
def detect_max_jump_after_stable(
    video_path: str,
    hash_size: int = 16,
    stable_distance_threshold: int = 2,  # 静止期：phash 距离 <= 1
    min_stable_frames: int = 3,  # 静止期最少连续帧数（关键帧对）
) -> int:
    """
...
def detect_video_frame_jump(ep_videos: list[list[str | Path]]) -> list[int]:
    for 
    return []


def detect_stationary_motion(ep_videos: list[str | Path], motion_features: set[str]) -> list[int]:
    return []
