...
from .mujoco_fk import MujocoReplay
...
class LerobotFkSolver:
    def __init__(self, config: LerobotFkSolverConfig, repo_path: str | Path) -> None:
        ...
        try:
            self.mujoco_fk_solver = MujocoReplay(
    ...
    def _embed_episode_fk_sim_data(
        self, episode_idx: int, state_data: list[np.ndarray], action_data: list[np.ndarray]
    ) -> None:
        if state_data is None and action_data is None:
            return
        if episode_idx >= len(self.sorted_parquet_files):
            raise Exception(f"Episode index {episode_idx} is out of range.")
        ...
        if state_data is not None:
            df["state_eef_pos_and_rot_euler"] = state_data
        if action_data is not None:
            df["action_eef_pos_and_rot_euler"] = action_data
