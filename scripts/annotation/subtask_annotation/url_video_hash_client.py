...
from robocoin_dataset.annotation.subtask_annotion.url_video_hash import UrlVideoHashClient
...
async def run_client_process(
    ...
    logger = setup_logger(
        name=f"url_video_hash_client_{process_id}",
        log_dir=log_path,
        level=logging.ERROR,
    )

    client = UrlVideoHashClient(
