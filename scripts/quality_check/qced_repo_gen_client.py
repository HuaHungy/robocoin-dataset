...
from robocoin_dataset.quality_check.qced_repo_generator import QualityCheckedRepoGeneratorClient
...
async def run_client_process(
    ...
    logger = setup_logger(
        name=f"quality_check_client{process_id}",
        log_dir=log_path,
        level=logging.ERROR,
    )

    client = QualityCheckedRepoGeneratorClient(
...
"""Usage:
python scripts/quality_check/qced_repo_gen_client.py \
    --host=127.0.0.1 \
    --port=2110 \
    --heartbeat-interval=10.0 \
    --log_dir=./logs/qced_repo_gen_client \
    --num-clients=8
"""
